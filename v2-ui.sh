...
install_bbr() {
    bash <(curl -L -s https://github.com/quniu/v2ui/raw/master/bbr.sh)
    if [[ $? == 0 ]]; then
        echo ""
...
update_shell() {
    wget -O /usr/bin/v2-ui -N --no-check-certificate https://github.com/quniu/v2ui/raw/master/v2-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
show_menu() {
  echo -e ""
  echo -e "${green}v2-ui 面板管理脚本${plain}"
  echo -e ""
  echo -e "
  ${green}0.${plain} 退出脚本
————————————————
